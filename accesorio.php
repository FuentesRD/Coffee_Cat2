...
$subcategoria_actual = 'Accesorio'; // Definimos la subcategoría para esta página
...
$fabricantes_disponibles = []; // Para filtro dinámico de fabricantes (opcional)
...
try {
    ...
    // --- Opcional: Obtener Fabricantes Disponibles para Filtros ---
    // $stmt_fabricantes = $pdo->prepare("SELECT DISTINCT fabricante FROM producto WHERE subcategoria = :subcategoria AND fabricante IS NOT NULL AND fabricante <> '' ORDER BY fabricante ASC");
    // $stmt_fabricantes->bindParam(':subcategoria', $subcategoria_actual, PDO::PARAM_STR);
    // $stmt_fabricantes->execute();
    // $fabricantes_disponibles = $stmt_fabricantes->fetchAll(PDO::FETCH_COLUMN);


    // --- Construir Consulta SQL Base y Filtros ---
    $sql_base = "FROM producto WHERE subcategoria = :subcategoria_base"; 
    $where_clauses = [];
    $params_to_bind = [':subcategoria_base' => $subcategoria_actual]; 
    ...
    $log_message = "Error de base de datos en accesorios.php: " . $e->getMessage() . PHP_EOL;
    ...
    // Descomentar para depuración en desarrollo:
...
<head>
    ...
    <link rel="stylesheet" href="molido.css"> <title>Accesorios de Café - Coffee Cat</title>
</head>
<body>
                ...
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li><a class="dropdown-item" href="accesorios.php"> 
        ...
        <div class="row">
            ...
            <aside class="col-lg-3 mb-4">
                 <div class="sticky-top" style="top: 100px;"> <h4><i class="bi bi-funnel"></i> Filtrar Productos</h4>
                    <hr>
                    <form method="GET" action="accesorios.php" id="filterForm">
                        <?php if (!empty($origenes_disponibles)): // Solo mostrar filtro de origen si hay orígenes para accesorios ?>
                        <div class="mb-4">
                            <h6><i class="bi bi-globe-americas"></i> Origen</h6>
                                <?php foreach ($origenes_disponibles as $origen_item): ?>
                        ...
                        <div class="mb-4">
                            ...
                            <?php /* Opcional: si quieres un dropdown para fabricantes
                            <?php if (!empty($fabricantes_disponibles)): ?>
                                <select name="fabricante" class="form-select mt-2">
                                    <option value="">Todos los fabricantes</option>
                                    <?php foreach($fabricantes_disponibles as $fab): ?>
                                        <option value="<?php echo htmlspecialchars($fab); ?>" <?php echo ($filtro_fabricante_get == $fab) ? 'selected' : ''; ?>>
                                            <?php echo htmlspecialchars($fab); ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            <?php endif; ?>
                        ...
                        <a href="accesorios.php" class="btn btn-outline-secondary w-100 mt-2">
            ...
            <section class="col-lg-9">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h1 class="display-6 mb-0"><i class="bi bi-funnel-fill"></i> Accesorios para Café</h1>
                                ...
                                <div class="card h-100 product-card">
                                    ...
                                    <div class="card-body d-flex flex-column">
                                        <div class="mb-2">
                                            ...
                                            <?php if (!empty($producto['origen'])): // Solo mostrar origen si existe para el accesorio ?>
                                            <br>
                                             <small class="text-muted">
                                                 <i class="bi bi-geo-alt-fill"></i> <?php echo htmlspecialchars($producto['origen']); ?>
                                             </small>
                                            <?php endif; ?>
                                        </div>
                    ...
                    <?php else: ?>
                        <div class="col-12">
                            ...
                            <div class="alert alert-info text-center">
                                <i class="bi bi-info-circle me-2"></i>No se encontraron accesorios que coincidan con los filtros seleccionados.
                            </div>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
